...
const CONFIG_PATH = path.join(process.cwd(), '.claude', 'verification-config.json');
...
**Config File:** .claude/verification-config.json
`;
  }
  ...
  static shouldVerify(operationType) {
    ...
    const category = operationMap[operationType] || 'other';
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { verifyCommand, VerificationController, VERIFICATION_MODES };
}

// CLI execution
if (require.main === module) {
  const args = process.argv.slice(2);
  verifyCommand(args).then(console.log).catch(console.error);
}